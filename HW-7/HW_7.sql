...
SELECT tableID AS 'номер стола', Capacity AS 'посадочные места' FROM Tables
JOIN Restaurant
ON Tables.RestaurantID = Restaurant.RestaurantID
AND IsAvailable = TRUE
AND Restaurant.RestaurantID = 2;
...
SELECT ReservationID, Customers.Name, TableID, ReservationTime FROM Reservations, Restaurant, Customers
WHERE Reservations.CustomerID = Customers.CustomerID
AND Restaurant.RestaurantID = 3;
...
SELECT WaiterID, Waiters.Name FROM Waiters
JOIN Restaurant
ON Waiters.RestaurantID = Restaurant.RestaurantID
AND Restaurant.RestaurantID = 4;
...
DELETE Customers, Reservations FROM Reservations
JOIN Customers
ON Reservations.CustomerID = Customers.CustomerID
WHERE Customers.CustomerID = 6;

/* добавляет клиента с ID 6 обратно после выполнения 6 задачи, нужно раскомментировать сроки ниже */
-- INSERT INTO Customers (CustomerID, Name, Email, Phone)
-- VALUES
--     (6, 'Olivia Miller', 'oliviamiller@email.com', '+6667778889');
-- INSERT INTO Reservations (ReservationID, TableID, CustomerID, ReservationTime)
...
SELECT COUNT(TableID) AS 'общее кол-во столов RestaurantID = 8' FROM Tables
...
-- + добавил названия ресторанов
SELECT Name, TableID, Capacity FROM Tables
JOIN Restaurant
ON Tables.RestaurantID = Restaurant.RestaurantID
WHERE Capacity >= 6;
